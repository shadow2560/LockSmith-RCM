...
static void _save_keys_to_sd(key_storage_t *keys, titlekey_buffer_t *titlekey_buffer, bool is_dev) {
	...
	u32 text_buffer_size = MAX(_titlekey_count * sizeof(titlekey_text_buffer_t) + 1, SZ_32K); // TODO SZ_16K before is 0x4000 (16 KO) so need to extend size if global keys need more than this size, now use SZ_32K
...
void dump_keys(bool no_display) {
	minerva_change_freq(FREQ_1600);
	...
	if (no_display) {
		...
		save_screenshot_and_go_back("dump_keys");
	}
}
...
void dump_keys_prodinfogen(key_storage_t* output) {
	...
	if (h_cfg.t210b01) {
		...
		se_get_aes_keys(aes_keys + 0x800, aes_keys, SE_KEY_128_SIZE);
		memcpy(&dev_keys.tsec_root_key, aes_keys + 11 * SE_KEY_128_SIZE, SE_KEY_128_SIZE);
		memcpy(keys->tsec_key, aes_keys + 12 * SE_KEY_128_SIZE, SE_KEY_128_SIZE);
		memcpy(&prod_keys.tsec_root_key, aes_keys + 13 * SE_KEY_128_SIZE, SE_KEY_128_SIZE);
		free(aes_keys);
