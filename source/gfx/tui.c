...
void *tui_do_menu(menu_t *menu)
{
	...
	while (true)
	{
		...
		// Skip caption or seperator lines selection or disabled item.
		while (menu->ents[idx].type == MENT_CAPTION ||
			menu->ents[idx].type == MENT_CHGLINE ||
			!menu->ents[idx].enabled)
		{
		...
		for (cnt = 0; menu->ents[cnt].type != MENT_END; cnt++) {
			if (!menu->ents[cnt].enabled) {
				gfx_con_setcol(0xFF555555, 1, 0xFF1B1B1B);
			} else if (cnt == idx) {
			...
			if (menu->ents[cnt].type != MENT_CHGLINE) {
				...
				} else if (!menu->ents[cnt].enabled) {
					gfx_printf("%k %s", 0xFF555555, menu->ents[cnt].caption);
				} else {
		...
		if (btn == (BTN_VOL_UP | BTN_VOL_DOWN)) {
			int res = save_fb_to_bmp("menu");
			u32 cx, cy;
			gfx_con_getpos(&cx, &cy);
			gfx_con_setpos(0, 1191);
			if (!res) {
				gfx_printf("%kScreenshot saved to sd:/LockSmith-RCM/screenshots/menu.bmp%k           ", COLOR_CYAN_L, COLOR_SOFT_WHITE);
			} else {
