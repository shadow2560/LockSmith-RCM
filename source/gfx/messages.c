...
#include "messages_packed.h"

 const char *log_msg_get(log_msg_id_t id) {
    if ((unsigned)id >= (unsigned)LOG_MSG_COUNT)
        return "";

    // logmsg_off_t is generated by tools/pack_assets (uint16_t or uint32_t).
    size_t off = (size_t)g_logmsg_offsets[(unsigned)id];
    if (off >= (size_t)g_logmsg_pool_size)
        return "";

    return (const char *)g_logmsg_pool + off;
}

u32 log_color_from_level(u8 lvl) {
	switch (lvl) {
...
const char * const g_log_messages[LOG_MSG_COUNT] = {
	...
	[LOG_MSG_BACK_TO_MENU]   = "Press any button to return to menu.",
	[LOG_MSG_CONTINUE]   = "Press any key to continue",
	[LOG_MSG_PRESS_KEY_TO_SHUTDOWN]   = "Press a button to shutdown the console.",
	...
	[LOG_MSG_ERR_FILE_COPY]   = "Error during file copy",
	[LOG_MSG_TITLE_SYSNAND]   = "[LockSmith-RCM v%d.%d.%d] - SYSNAND WORK\n\n",
	[LOG_MSG_TITLE_EMUNAND]   = "[LockSmith-RCM v%d.%d.%d] - EMUNAND WORK\n\n",
	[LOG_MSG_ERR_PAYLOAD_FILE_ERROR]   = "Payload file is missing!\n(%s)",
	[LOG_MSG_ERR_PAYLOAD_TOO_BIG]   = "Payload is too big!",
	[LOG_MSG_ERR_NO_PAYLOAD_FOUND]   = "No payloads or modules found.",
	[LOG_MSG_ERR_PAYLOAD_LAUNCH]   = "Failed to launch payload.",
	[LOG_MSG_WARN_MINERVA]   = "Minerva not enabled, functions will be slower down.",
	[LOG_MSG_ERR_FUNCTIONS_AND_BATCH_DISABLED]   = "Most function will be disabled and batch will not be executed.",
	[LOG_MSG_ERR_BIS_KEYS_LOAD]   = "Failed to load BIS keys!",
	[LOG_MSG_ERR_NANDS_LOAD]   = "Failed to init EMMC or to discover an emunand config!",
	[LOG_MSG_BATCH_VIEW_LOG]   = "Press any key to view batch log.",
	...
	[LOG_MSG_PROPOSE_TAKE_SCREENSHOT]   = "\nPress VOL+ to save a screenshot or another button to return to the menu.\n",
	[LOG_MSG_TAKE_SCREENSHOT_SUCCESS]   = "Screenshot sd:/LockSmith-RCM/screenshots/%s saved.",
	[LOG_MSG_TAKE_SCREENSHOT_ERROR]   = "Screenshot save failed.",
	...
	[LOG_MSG_ERROR_PRODINFO_CRC_VERIF]   = "CRC verification error at 0x%x",
	[LOG_MSG_ERROR_PRODINFO_CRC_COMPARE]   = "CRC compare error at 0x%x",
	[LOG_MSG_ERROR_PRODINFO_SHA_VERIF]   = "SHA256 verification error at 0x%x",
	[LOG_MSG_ERROR_PRODINFO_SHA_COMPARE]   = "SHA256 compare error at 0x%x",
	...
	[LOG_MSG_ERR_FOLDER_READ]   = "Error when reading folder.",
	...
	[LOG_MSG_FILE_TRANSFERT_FROM_NANDS_ERR_READ_SRC]   = "Read error on source file.",
	...
	[LOG_MSG_FUSE_CHECK_TITLE]   = "NINTENDO SWITCH FUSE CHECKER",
	[LOG_MSG_FUSE_CHECK_FW]   = "Firmware: %d.%d.%d",
	[LOG_MSG_FUSE_CHECK_BURNT_FUSES]   = "Burnt Fuses: %d",
	[LOG_MSG_FUSE_CHECK_REQUIRED_FUSES]   = "Required Fuses: %d",
	[LOG_MSG_FUSE_CHECK_STATUS_UNDERBURNT_1]   = "STATUS: WARNING (UNDERBURNT)",
	[LOG_MSG_FUSE_CHECK_STATUS_UNDERBURNT_2]   = "Missing extra %d burnt fuse(s) - will be burnt on next normal boot!",
	[LOG_MSG_FUSE_CHECK_STATUS_UNDERBURNT_3]   = "What will work without burning extra fuses: CFW (Atmosphere), Semi-stock (Hekate nogc)",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_1]   = "STATUS: ERROR (OVERBURNT)",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_2]   = "Extra %d fuse(s) burnt - OFW WILL NOT BOOT!",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_3]   = "System will black screen on OFW boot",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_4]   = "What will work: CFW (Atmosphere), Semi-stock (Hekate nogc)",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_5]   = "No upgrade to any known firmware possible.",
	[LOG_MSG_FUSE_CHECK_STATUS_OVERBURNT_6]   = "Upgrade at least to FW %d.%d.x to have a normal boot possible",
	[LOG_MSG_FUSE_CHECK_STATUS_OK_1]   = "STATUS: PERFECT MATCH",
	[LOG_MSG_FUSE_CHECK_STATUS_OK_2]   = "Exact fuse count match - OFW AND CFW WILL BOOT NORMALLY",
	...
	[LOG_MSG_WIP_SUCCESS] = "Wip done, please reboot with Atmosphere one time on the nand treated then reboot normaly when the config screen is loaded",
	[LOG_MSG_DG_SUCCESS] = "Downgrade fix done",
	[LOG_MSG_RM_ERPT_WARN] = "Very dangerous operation, all your installed game will not launch after that so do it only if someone told you to do it.",
	[LOG_MSG_RM_ERPT_SUCCESS] = "ERPT deletion done",
	[LOG_MSG_RM_PARENTAL_CONTROL_SUCCESS] = "Parental control deletion done",
	[LOG_MSG_SYNCH_JOYCONS_INFOS_1] = "This will correct the joysticks synchronization problems between nands.\nBefore using this script, connect all your controllers to the source nand and backup the file \"SYSTEM:/save/8000000000000050\" of your dest nand in case of a problem.\n",
	[LOG_MSG_SYNCH_JOYCONS_INFOS_2] = "If you use 90DNS or any other DNS setting, please switch to airplane mode on the two nands cause this script will copy also the wifi settings from your source nand to the dest nand.\n\nAlso note that some of your dest nand settings will be overwritten, such as the theme, so check the parameters of the ddest nand after the execution of this script.\n",
	...
	[LOG_MSG_PRODINFOGEN_DISPLAY_DEVICE_ID]   = "Your device id: %s",
	[LOG_MSG_PRODINFOGEN_DISPLAY_KEY_GENERATION]   = "key generation: %d\n",
	...
	[LOG_MSG_PRODINFOGEN_DONOR_INFOS]   = "Donor PRODINFO looks valid\n version = %d\n device id = %s\n",
	...
	[LOG_MSG_PRODINFOGEN_KEYS_DUMPING]   = "Dumping keys...",
	...
	[LOG_MSG_PRODINFOGEN_GAMECARD_CERT_IMPORT]   = "Importing GameCard certificate",
	[LOG_MSG_PRODINFOGEN_EXTENDED_GAMECARD_key_IMPORT]   = "Importing extended GameCard key",
	...
	[LOG_MSG_MARIKO_KEYS_DUMP_INFOS_1]   = "This dumps the results of writing zeros over consecutive 32-bit portions of each keyslot, the results of which can then be bruteforced quickly on a computer to recover keys from unreadable keyslots.\n\nThis includes the Mariko KEK and BEK as well as the unique SBK.\n\nThese are not useful for most users but are included for archival purposes.",
	[LOG_MSG_MARIKO_KEYS_DUMP_INFOS_2]   = "Warning: this wipes keyslots!\nThe console must be completely restarted!\nModchip must run again to fix the keys!\n",
	[LOG_MSG_MARIKO_KEYS_DUMP_VALIDATE_KEYSLOT_ERROR]   = "Failed to validate keyslot %d.",
	[LOG_MSG_MARIKO_KEYS_DUMP_OVERWRITE_KEYSLOT_ERROR]   = "Failed to overwrite keyslot %d.",
	[LOG_MSG_MARIKO_KEYS_DUMP_DUMP_KEY_ERROR]   = "Failed to dump partial keys %d-%d.",
	[LOG_MSG_MARIKO_KEYS_DUMP_WRITE_KEY_ERROR]   = "Unable to write partial keys to SD.",
	[LOG_MSG_MARIKO_KEYS_DUMP_SUCCESS]   = "Wrote mariko partials keys to %s",
	[LOG_MSG_DUMP_FW_BEGIN]   = "Firmware dump begin...",
	[LOG_MSG_DUMP_FW_DIR_REPLACE_ASK]   = "Destination already exists. Press vol+ to replace or any key to cancel.",
	[LOG_MSG_DUMP_FW_ERROR]   = "Error during firmware dump",
	[LOG_MSG_DUMP_FW_END]   = "Firmware dump done in %ds",
	[LOG_MSG_INCOGNITO_RETRY_DEF]   = "Retry %d/%d...",
	[LOG_MSG_INCOGNITO_ERR_WRITE]   = "ERROR, ERRO! Length is %d!",
	[LOG_MSG_INCOGNITO_VERIF_BEGIN]   = "Verifying  CAL0 hashes and CRCs...",
	[LOG_MSG_INCOGNITO_VERIF_ERR]   = "Verification not successful!",
	[LOG_MSG_INCOGNITO_VERIF_SUCCESS]   = "Verification successful! Serial:%s",
	[LOG_MSG_INCOGNITO_ERR]   = "Incognito error!",
	[LOG_MSG_INCOGNITO_SUCCESS]   = "Incognito done!",
...
void log_printf(bool record_message, log_level_t lvl, log_msg_id_t id, ...) {
	const char *fmt = log_msg_get(id);
	bool store = (record_message) && (called_from_config_files && !called_from_AIO_LS_Pack_Updater && g_log_count < LOG_MAX_ENTRIES);
	...
	u32 color = log_color_from_level(lvl);
	gfx_printf("%k", color);
...
void log_export_txt(const char *path) {
	...
	for (u32 i = 0; i < g_log_count; i++) {
		...
		const char *fmt = log_msg_get((log_msg_id_t)e->msg_id);
